...
```ts
await storage.getItem<string>("k");
// => <string>
await storage.getItemRaw<string>("k");
// => <string>
```

Checks setItem value type.
...
Multiple types can also be categorized using `prefixStorage`.
...
await htmlStorage.getItem("foo.html");
// => <string>
...
await postStorage.getItem("foo.json");
// => <Post>
```
...
::alert{type="warning"}
Currently maybe null or undefined when miss getItem, Please check [unjs/unstorage#205](https://github.com/unjs/unstorage/issues/205) for more information.
::
