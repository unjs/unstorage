    ...
    async getItem (key) {
      ...
      if (!item.body) {
        try {
          ...
          item.body = await $fetch(`/api/v4/projects/${encodeURIComponent(opts.repo!)}/repository/files/${encodeURIComponent(path)}/raw?ref=${opts.branch}`, {
            baseURL: opts.apiURL,
...
async function fetchFiles (opts: GitlabOptions) {
  ...
  try {
    const trees = await $fetch(
      `/api/v4/projects/${encodeURIComponent(opts.repo!)}/repository/tree?recursive=1`,
      {
        baseURL: opts.apiURL,
        headers: opts.headers
      }
